...
b = 1; a = 0.3; w = 4;    % try 25;        % smooth wobbly radial shape params
R = @(t) b*(1 + a*cos(w*t));
R_t = @(t) -a*b*w*sin(w*t); 

N = 4e4;         % overkill by factor 20, but good for test evalmeth='f'.
evalmeth = 'f';   % summation method for pot eval: 'd' direct slow, 'f' FMM
P = 200;         % discretization in phi direction 
