...
clear; v = 1;        % verbosity: 0 = text only, 1 = total potential plot
...
if v>1, nam = 'amoeba_spin';  % movie
  wO=VideoWriter([nam '.avi']); wO.FrameRate=20; wO.open;
  dx = 0.01; gx = -3:dx:2; gy = -2:dx:2;       % image plot grid  
  ...
  for ti = (0:nti-1)/nti*2*pi
    ...
    ur = mfseval(k,[real(t.x)';imag(t.x)'],cx,co,evalmeth);  % apply A to co
    rrms = norm(ur(:) - rhs)/sqrt(M);
    fprintf('th=%.3g: resid rms = %.3g \t co 2-nrm = %.3g\n',ti,rrms,nrm)
